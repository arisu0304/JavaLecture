...
public class _03_String02 {
	...
	public static void main(String[] args) {
		...
		// String은 space값도 문자열값으로 인식(인덱스로 잡힌다.)
		String str3 = "abcdha bcdeba";
		
		// 1. contains: 문자열을 포함하는지 여부를 검사하는 메소드
		// 포함하고 있으면 true, 포함하지 않으면 false를 리턴
		System.out.println(str2.contains("tP"));
		System.out.println(str3.contains("tP"));
		
		// 2. indexOf, lastIndexOf : 특정 문자나 문자열의 인덱스를 리턴하는 메소드
		// 포함되어 있으면 해당 문자의 인덱스 찾아서 리턴, 포함되어 있지 않으면 -1 리턴
		// indexOf는 왼쪽에서 오른쪽으로 검사
		// lastIndexOf는 오른쪽에서 왼쪽으로 검사
		System.out.println("str2에서 h의 인덱스는 " + str2.indexOf('h'));
		System.out.println("str3에서 a의 인덱스는 " + str3.indexOf('a'));
		
		// 검사의 시작위치 지정가능 - 오버로딩 되어있음
		System.out.println("str3에서 인덱스 4부터 a의 인덱스는"+
							 str3.indexOf('a',4));
		
		System.out.println("str3에서 abc의 인덱스는 " + str3.indexOf("abc"));
		System.out.println("str2에서 abc의 인덱스는 " + str2.indexOf("abc"));
		
		System.out.println("str3에서 b의 마지막 인덱스는 " +  str3.lastIndexOf('b'));
		System.out.println("str3에서 bc의 마지막 인덱스는 " + str3.lastIndexOf("bc"));
		...
		// 치환하는 메소드, replace와 replaceAll의 차이는 replaceALL은
		// 정규표현식을 매개변수로 전달할 수 있다는 점이다.
		...
		// 5. 체이닝 기법
		// 메소드를 체인처럼 엮어서 사용하는 것을 체이닝 기법이라고 한다.
