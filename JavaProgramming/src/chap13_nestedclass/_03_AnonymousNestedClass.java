...
public class _03_AnonymousNestedClass {
	...
	public static void main(String[] args) {
		...
		// 1. 익명 중첩 클래스
		// 익명 중첩 클래스는 인터페이스나 추상클래스를 상속받은 자식클래스를
		// 만들지 않고 객체를 생성할 때 사용한다.
		// 별도로 클래스를 생성하지 않기 때문에 클래스의 이름이 없고
		// 따라서 익명 중첩 클래스라고 한다.
		// 인터페이스나 추상 클래스는 객체를 생성할 수가 없지만
		// 인터페이스나 추상 클래스의 생성자를 호출하면서 추상메소드를 모두 구현해주면
		// 상속받아 구현된 자식 클래스 없이도 인터페이스나 추상클래스가 구현된 
		// 객체를 생성할 수 있게 된다.
