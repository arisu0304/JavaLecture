...
// 중첩클래스를 감싸고 있는 클래스를 외부 클래스라고 부르며 중첩클래스는 내부 클래스라고도 한다.
// 중첩클래스의 종류는 정적 중첩 클래스, 인스턴스 중첩 클래스, 지역 중첩 클래스, 익명 중첩 클래스
...
public class NestedClass {
	...
	// 외부 클래스의 멤버변수나 인스턴스 메소드는 객체가 생성된 후에 사용할 수 있기 때문이다.
	// 외부 클래스의 객체 생성 없이도 객체를 만들 수 있다.
	public static class Mul {
		...
		// 5-3. 정적 중첩 클래스의 인스턴스 메소드
			public int mul(int a, int b) {
				...
				// private이어도 같은 클래스이므로 접근 가능
				num3 = 10;
				a = sub(num3, 5);
				return a*b;
			}
			
		// 5-4. 정적 중첩 클래스의 클래스 메소드
