...
public class Middle {
	...
	public static void main(String[] args) {
		...
		String[] strArr2 = {bit, com , sql , intStr};
		
		char[] charArr2 = new char[12];
		
		for(int i = 0 ;  i < 12 ; i++) {
			if( i % 4 == 0) {
				charArr2[i] = strArr2[0].charAt(i / 4);
			}else if( i % 4 == 1) {
				charArr2[i] = strArr2[1].charAt(i / 4);
			}else if( i % 4 == 2) {
				charArr2[i] = strArr2[2].charAt(i / 4);
			}else{
				charArr2[i] = strArr2[3].charAt(i / 4);
			}
			// 위에는 for j를 0~3으로 돌려서 줄여도 ok
